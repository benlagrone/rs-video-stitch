...
function MotionPlanEditor({ plan, busy, onPlan, onSave, onRegionChange }) {
  ...
  return <section className="motion-plan-editor"><div className="motion-plan-heading"><div><strong>Structured motion plan</strong><span>{plan.summary}</span><small>Object vectors preserve and tween the original subject. Generative regions are reserved for fire, water, smoke, light, terrain, and atmosphere. The unselected background stays fixed.</small></div><div><button type="button" className="secondary-action" onClick={onSave} disabled={busy}>Save plan</button><button type="button" className="secondary-action" onClick={onPlan} disabled={busy}>{busy ? 'Replanning…' : 'Replan from prompt'}</button></div></div><div className="motion-region-list">{(plan.regions || []).map((region, index) => <div className="motion-region" key={region.id || index}><label className="motion-region-toggle"><input type="checkbox" checked={region.enabled !== false} onChange={(event) => onRegionChange(index, { enabled: event.target.checked })} />{region.label}</label><input aria-label={`${region.label} action`} value={region.action || ''} onChange={(event) => onRegionChange(index, { action: event.target.value })} /><label>Method<select aria-label={`${region.label} method`} value={region.method || 'generative-region'} onChange={(event) => onRegionChange(index, { method: event.target.value })}><option value="object-vector">Exact object vector</option><option value="generative-region">Generative region</option></select></label><label>Direction<select aria-label={`${region.label} direction`} value={region.direction || 'right'} onChange={(event) => onRegionChange(index, { direction: event.target.value })}><option value="left">Left</option><option value="right">Right</option><option value="up">Up</option><option value="down">Down</option><option value="outward">Outward</option><option value="clockwise">Clockwise</option><option value="counterclockwise">Counterclockwise</option><option value="pulse">Pulse</option></select></label>{(region.method || 'generative-region') === 'object-vector' && <><label>Horizontal vector<input type="number" min="-0.5" max="0.5" step="0.01" value={region.vector?.dx ?? 0} onChange={(event) => onRegionChange(index, { vector: { ...(region.vector || {}), dx: Number(event.target.value) } })} /></label><label>Vertical vector<input type="number" min="-0.5" max="0.5" step="0.01" value={region.vector?.dy ?? 0} onChange={(event) => onRegionChange(index, { vector: { ...(region.vector || {}), dy: Number(event.target.value) } })} /></label></>}<label>Strength<input type="range" min="0.1" max="1" step="0.05" value={region.strength || 0.5} onChange={(event) => onRegionChange(index, { strength: Number(event.target.value) })} /></label></div>)}</div><small className="motion-route-policy">Full-frame regeneration is off by default. If a regional method fails, MediaStudio preserves the still instead of inventing a replacement scene.</small></section>;
}
...
export function BibleStudio({ authToken, theme, initialProjectId = '', onBack, onOpenProjects }) {
  ...
  async function planSceneMotion(sceneIndex, regenerate = true) {
    ...
    try {
      const result = await request(`/v1/projects/${encodeURIComponent(project.projectId)}/scenes/${sceneIndex}/motion-plan`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ regenerate, motionPlan: regenerate ? null : motionPlans[sceneIndex], prompt: animationPrompts[sceneIndex] || '' }),
      });
  ...
  async function repairAndAnimateScene(sceneIndex) {
    if (!project?.projectId) return;
    setError('');
    try {
      const created = await request(`/v1/projects/${encodeURIComponent(project.projectId)}/scenes/${sceneIndex}/repair-and-animate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      <form className="bible-workspace" onSubmit={buildVideo}>
        ...
        <section className="bible-storyboard">
          ...
          {scenes.length ? <div className="bible-scene-list">{scenes.map((scene, index) => {
            ...
            const needsMotionSafeStill = /clipped by the source frame|motion-safe repair could not produce/i.test(rejectionReason || '');
